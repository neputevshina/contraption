...
func (c *Context) TextRune(fs *fontstashmini.FontStash, x, y float32, runes []rune) float32 {
	...
	for {
		...
		if iter.PrevGlyph == nil || iter.PrevGlyph.Index == -1 {
			_, b := c.AllocTextAtlas(c.fs, c.fontImageIdx)
			if !b {
				break // no memory :(
			}
			if index != 0 {
	...
	c.FlushTextTexture(c.fs, c.fontImageIdx)
	c.RenderText(vertexes[:index])
	return iter.X
}
...
// 			break
// 		}
// 		if iter.PrevGlyph.Index == -1 && !c.AllocTextAtlas(c.fs, c.fontImageIdx) {
// 			iter = prevIter
// 			quad, _ = iter.Next() // try again
// 		}
// 		prevIter = iter
...
// 			break
// 		}
// 		if iter.PrevGlyph == nil || iter.PrevGlyph.Index == -1 && !c.AllocTextAtlas(c.fs, c.fontImageIdx) {
// 			iter = prevIter
// 			quad, _ = iter.Next() // try again
// 		}
// 		prevIter = iter
...
func createInternal(params nvgParams) (*Context, error) {
	context := &Context{
		...
		cache: nvgPathCache{
			...
			vertexes: make([]Vertex, 0, nvgInitVertsSize),
		},
	}
	context.Save()
	context.Reset()
...
func (c *Context) AllocTextAtlas(fs *fontstashmini.FontStash, hfont int) (newhfont int, ok bool) {
	c.FlushTextTexture(fs, hfont)
	if hfont >= nvgMaxFontImages-1 {
		return hfont, false
	}
	var iw, ih int
...
func (c *Context) RenderText(vertexes []Vertex) {
	state := c.getState()
	paint := state.fill
