...
import (
	...
	"github.com/neputevshina/contraption/nanovgo/fontstashmini/truetype"
...
func (stash *FontStash) LineBounds(y float32) (minY, maxY float64) {
	state := stash.state
	if len(stash.fonts) < state.font+1 {
		return -1, -1
	}
	...
	minY = float64(y - font.ascender*iSize/10.0)
	maxY = minY + float64(font.lineh*iSize/10.0)
	return
}
